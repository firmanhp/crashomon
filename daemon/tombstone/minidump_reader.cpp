...
std::string ReadUtf8Str(const std::string& buf, uint32_t rva) {
  ...
  if (length == 0 || rva + 4 + length > buf.size()) {
    return {};
  }
...
void ExtractCrashpadAnnotations(const std::string& path, MinidumpInfo& info) {
  ...
  if (buf.size() < kMdmpHeaderSize) {
    return;
  }
  if (ReadU32Le(buf, 0) != kMdmpSignature) {
    return;
  }
  ...
  if (stream_rva + kCrashpadInfoMinSize > buf.size()) {
    return;
  }
  if (ReadU32Le(buf, stream_rva) != 1U) {
    return;
  }
  ...
  if (dict_rva == 0 || dict_size < kDictCountSize) {
    return;
  }
  if (dict_rva + dict_size > buf.size()) {
    return;
  }
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (entry_off + kDictEntrySize > buf.size()) {
      break;
    }
